...
function buildHTMLCells(){
    ...
    for(let w = 0; w < 6; w++){
        ...
        for(let d = 0; d < 7; d++){
            ...
            cellDiv.setAttribute("onClick", `dateClicked(${w},${d})`)
...
function placeHTMLSheet(){
    ...
    let ad = getActiveDate();
    heading.innerText = CalendarTools.monthYearString(ad);

    wrapper.appendChild(heading);
    wrapper.appendChild(currentHTMLSheet);
    wrapper.setAttribute("id", CALENDAR_WRAPPER_ID)

    currentHTMLSheet.setAttribute("id", CALENDAR_SHEET_ID);
...
function getActiveDate(){ // to make a later switch to another system easier
    return activeDate;
}
